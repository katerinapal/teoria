...
var _daccord = require("daccord");

var _daccord2 = _interopRequireDefault(_daccord);
...
function Chord(root, name) {
  ...
  this.intervals = (0, _daccord2.default)(name).map(_interval2.Interval.toCoord);
  ...
  if (bass) {
    ...
    note = _note.Note.fromString(bass + (root.octave() + 1)); // crude

    bassInterval = _interval2.Interval.between(root, note);
...
Chord.prototype = {
  ...
  voicing: function voicing(_voicing) {
    ...
    for (var i = 0, length = _voicing.length; i < length; i++) {
      this._voicing[i] = _interval2.Interval.toCoord(_voicing[i]);
    }
  ...
  get: function get(interval) {
    if (typeof interval === 'string' && interval in _knowledge.knowledgejs.stepNumber) {
      var intervals = this.intervals,
          i,
          length;

      interval = _knowledge.knowledgejs.stepNumber[interval];
