var mod_Chord = Chord;
import ext_daccord_daccord from "daccord";
import { knowledgejs as knowledge } from "./knowledge";
import { Note as note_Note } from "./note";
...
function Chord(root, name) {
  ...
  this.intervals = ext_daccord_daccord(name).map(interval_Interval.toCoord)
  ...
  if (bass) {
    ...
    note = note_Note.fromString(bass + (root.octave() + 1)); // crude

    bassInterval = interval_Interval.between(root, note);
...
Chord.prototype = {
  ...
  voicing: function(voicing) {
    ...
    for (var i = 0, length = voicing.length; i < length; i++) {
      this._voicing[i] = interval_Interval.toCoord(voicing[i]);
    }
