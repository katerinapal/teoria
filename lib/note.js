var mod_Note = Note;
import ext_scientific from "scientific-notation";
import ext_helmholtz_helmholtz from "helmholtz";
import ext_pitchFq from "pitch-fq";
import { knowledgejs as knowledge } from "./knowledge";
import { vectorjs as vector } from "./vector";
import { Interval as interval_Interval } from "./interval";
...
Note.prototype = {
  ...
  */
  ...
  fq: function(concertPitch) {
    return ext_pitchFq(this.coord, concertPitch);
  },
  ...
  interval: function(interval) {
    if (typeof interval === 'string') interval = interval_Interval.toCoord(interval);

    if (interval instanceof interval_Interval)
      return new Note(vector.add(this.coord, interval.coord));
    else if (interval instanceof Note)
      return new interval_Interval(vector.sub(interval.coord, this.coord));
  },
  ...
  scaleDegree: function(scale) {
    ...
    return scale.scale.reduce(function(index, current, i) {
      var coord = interval_Interval.toCoord(current).coord;
...
Note.fromString = function(name, dur) {
  var coord = ext_scientific(name);
  if (!coord) coord = ext_helmholtz_helmholtz(name);
